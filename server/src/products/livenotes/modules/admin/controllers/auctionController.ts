...
export const getAllAuctionRequests = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const requests = await AuctionRequest.find(query)
      ...
      .populate('propertyOwnerId', 'name email phone')
...
// @desc    Get auction request by ID (Admin)
// @route   GET /api/v1/admin/auction-requests/:id
// @access  Private (Admin)
export const getAuctionRequestById = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
// @desc    Approve auction request (Admin)
// @route   POST /api/v1/admin/auction-requests/:id/approve
// @access  Private (Admin)
...
// @desc    Decline auction request (Admin)
// @route   POST /api/v1/admin/auction-requests/:id/decline
// @access  Private (Admin)
...
// @desc    Delete auction request (Admin)
// @route   DELETE /api/v1/admin/auction-requests/:id
// @access  Private (Admin)
export const deleteAuctionRequest = async (
  ...
  try {
    const request = await AuctionRequest.findByIdAndDelete(req.params.id)
    ...
    res.status(200).json({
      status: 'success',
      message: 'Auction request deleted successfully',
    })
  } catch (error) {
    next(error)
  }
